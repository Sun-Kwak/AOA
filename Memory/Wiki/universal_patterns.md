...
**실수**:
- AOA 저장소가 여러 곳에 클론됨
- AI가 어떤 경로를 사용해야 하는지 헷갈림
- **중복 클론에서 작업** → 프로젝트 파일 접근 불가

**결과**:
- Wiki Protocol 무용지물
- 매 세션마다 동일한 실수 반복
- 프로젝트 경로 불일치로 워크플로우 실행 불가
...
**모든 작업 시작 전:**
```bash
# 현재 위치가 올바른 AOA 저장소인지 확인
git rev-parse --show-toplevel
# → /Users/sun/project/AOA 또는 워크트리 경로여야 함
# → 다른 경로면 중복 클론 → 작업 중단
```

**중복 제거:**
- ✅ 하나의 AOA 저장소만 유지
- ❌ `/Users/sun/StudioProjects/AOA` 같은 중복 클론 금지
- ✅ GitHub Copilot 프로젝트 등록 경로와 작업 경로 일치 필수
